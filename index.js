...
* KASIH CREDITS LAH BOSQUE T027BOT
*/
const
{
   WAConnection,
...
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN?
            ...
            + 'TEL;type=CELL;type=VOICE;waid=6281220439155:+62 877-7545-2636\n' // JANGAN KEK BABI SU
            + 'END:VCARD' // T027BOT
prefix = '.'
blocked = []            
...
const config = {
    A187: '𝐒𝐄𝐋𝐅𝐁𝐎𝐓', // TOBAT SU ASU
    instagram: 'https://instagram.com/nfz.01', // INFO JANGAN DI UBAH
    nomer: 'wa.me/6281220439155', // INFO SU JNGAN DI UBAH
    youtube: 'https://bit.ly/2KPPAnB', // KINTIL
    whatsapp: 'https://bit.ly/34zXWH9', // BABI
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `𝗛𝗮𝗹𝗹𝗼 @${num.split('@')[0]}\𝘀𝗲𝗹𝗮𝗺𝗮𝘁 𝗱𝗮𝘁𝗮𝗻𝗴 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽 *${mdata.subject}* 𝘆𝗮𝗻𝗴 𝗯𝗲𝘁𝗮𝗵 𝘆𝗮 𝗱𝗶𝘀𝗶𝗻𝗶`
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				success: '️❬ ✔ ❭ 𝘀𝘂𝗰𝗰𝗲𝘀𝘀',
				error: {
					stick: '𝗬𝗮𝗵 𝗴𝗮𝗴𝗮𝗹, 𝘂𝗹𝗮𝗻𝗴𝗶 𝗹𝗮𝗴𝗶 𝘆𝗮 𝘀𝗮𝘆𝗮𝗻𝗴',
					Iv: '𝗠𝗮𝗮𝗳 𝗹𝗶𝗻𝗸 𝘁𝗶𝗱𝗮𝗸 𝘃𝗮𝗹𝗶𝗱'
				},
				only: {
					group: '❬❗❭  𝗢𝗡𝗟𝗬',
					ownerG: '❬❗❭ 𝗢𝗪𝗡𝗘𝗥 𝗚𝗥𝗢𝗨𝗣 𝗢𝗡𝗟𝗬',
					ownerB: '❬❗❭  𝗢𝗪𝗡𝗘𝗥 𝗢𝗡𝗟𝗬',
					admin: '❬❗❭ 𝗔𝗗𝗠𝗜𝗡 𝗢𝗡𝗟𝗬',
			...
			const ownerNumber = ["6281220439155@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						ranp = getRandom('.png')
						reply(mess.wait)
						keyrmbg = '18r5GV2j14RV4qrVBHm2yJ4S'
				...
				case 'tts':
					...
					if (args.length < 2) return client.sendMessage(from, '𝗧𝗲𝗸𝘀𝗻𝘆𝗮 𝗺𝗮𝗻𝗮 𝘁𝗲𝗸𝘀?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
			...
			case 'loli': 
				...
				case 'nsfwloli': 
				    try {
						...
						buffer = await getBuffer(res.url)
						client.sendMessage(from, buffthr, image, {quoted: mek, caption: 'Jangan jadiin bahan buat comli'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
						reply('𝗘𝗥𝗥𝗢𝗥')
					}
				...
				case 'yt': 
					...
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/yta?url=${args[0]}&apiKey=gauCUw6Q51JOHcliTQr8`, {method: 'get'})
				...
				case 'ytsearch': 
					...
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/ytsearch?q=${body.slice(10)}&apiKey=gauCUw6Q51JOHcliTQr8`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = '=================\n'
				...
				case 'tiktok': 
					...
					reply(mess.wait)
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/tiktok?url=${args[0]}&apiKey=gauCUw6Q51JOHcliTQr8`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					buffer = await getBuffer(anu.result)
				...
				case 'tulis':
					...
					teks = body.slice(7)
					reply(mess.wait)
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/nulis?text=${teks}&apiKey=gauCUw6Q51JOHcliTQr8`, {method: 'get'})
					if (anu.error) return reply(anu.error)
				...
				case 'url2img': 
					...
					reply(mess.wait)
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/url2image?tipe=${args[0]}&url=${args[1]}&apiKey=gauCUw6Q51JOHcliTQr8`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					buff = await getBuffer(anu.result)
				...
				case 'tsticker': 
					...
					anu = await fetchJson(`https://mhankbarbars.herokuapp.com/api/text2image?text=${teks}&apiKey=gauCUw6Q51JOHcliTQr8`, {method: 'get'})
					if (anu.error) return reply(anu.error)
              ...
              costum('Ini', '6281220439155@s.whatsapp.com')
                  break	
				...
				case 'clearall':
					...
					break
				case 'leave': 
					if (!isOwner) return reply('𝗟𝘂 𝘀𝗶𝗮𝗽𝗮?') 	
					if (!isGroup) return reply(mess.only.group)
					await client.sendMessage(dari, '𝗕𝘆𝗲𝗲').then(() => client.leaveGroup(groupId))
                    break
				case 'bc': 
					if (!isOwner) return reply('𝙡𝙪 𝙨𝙞𝙖𝙥𝙖?') 
       ...
       client.sendMessage(from, '𝗧𝘂𝗵 𝗻𝗼𝗺𝗲𝗿 𝗼𝘄𝗻𝗲𝗿 𝗧𝟬𝟮𝟳𝗕𝗢𝗧,𝗝𝗔𝗡𝗚𝗔𝗡 𝗟𝗨𝗣𝗔 𝗗𝗜 𝗦𝗔𝗩𝗘',MessageType.text, { quoted: mek} )
           break    
           case 'demote':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `𝗬𝗮𝗵 𝗸𝗮𝗺𝘂 𝗱𝗶 𝘁𝘂𝗿𝘂𝗻𝗶𝗻 𝗷𝗮𝗱𝗶 𝗮𝗱𝗺𝗶𝗻 𝘀𝗮𝗺𝗮 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`𝗬𝗮𝗵 𝗸𝗮𝗺𝘂 𝗱𝗶 𝘁𝘂𝗿𝘂𝗻𝗶𝗻 𝗷𝗮𝗱𝗶 𝗮𝗱𝗺𝗶𝗻 𝘀𝗮𝗺𝗮 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `𝗦𝗲𝗹𝗮𝗺𝗮𝘁 𝗮𝗻𝗱𝗮 𝗱𝗶𝗷𝗮𝗱𝗶𝗸𝗮𝗻 𝗮𝗱𝗺𝗶𝗻 𝘀𝗮𝗺𝗮 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`𝗦𝗲𝗹𝗮𝗺𝗮𝘁 𝗮𝗻𝗱𝗮 𝗱𝗶𝗷𝗮𝗱𝗶𝗸𝗮𝗻 𝗮𝗱𝗺𝗶𝗻 𝘀𝗮𝗺𝗮 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `𝗔𝘀𝗲𝗸 𝗱𝗮𝗽𝗮𝘁 𝗺𝗮𝗸𝗮𝗻𝗮𝗻,𝗼𝘁𝘄 𝗸𝗶𝗰𝗸 𝗦𝗮𝘆𝗮𝗻𝗴 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`𝗔𝘀𝗲𝗸 𝗱𝗮𝗽𝗮𝘁 𝗺𝗮𝗸𝗮𝗻𝗮𝗻,𝗼𝘁𝘄 𝗸𝗶𝗰𝗸 𝗦𝗮𝘆𝗮𝗻𝗴 : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: '𝗡𝗶𝗵 𝘀𝗮𝘆𝗮𝗻𝗴'})
						fs.unlinkSync(ran)
					})
					break
				...
				case 'simih':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('𝗦𝗮𝘆𝗮𝗻𝗴')
					if (Number(args[0]) === 1) {
			...
			if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ERROR]','red'), 'Command Tidak Terdaftar di Menu!', color(sender.split('@')[0]))
					}
					}
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
